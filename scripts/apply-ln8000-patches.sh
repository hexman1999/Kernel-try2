...
# Applies LN8000, DTBO, LTO, and other optimizations
...
apply_patches() {
    for patch_url in "$@"; do
        ...
        if [ -s /tmp/temp_patch.patch ]; then
            if patch -s -p1 --fuzz=5 < /tmp/temp_patch.patch; then
...
echo "  - LN8000 charger patches: 5 patches"
echo "  - DTBO patches: 6 patches"
